...
import { fetchDeployment } from '@/lib/siteAdmin';
...
export default function DeploymentDetailPage() {
  ...
  const deploymentQuery = useQuery({
    ...
    queryFn: async () => (slug ? fetchDeployment(slug) : null),
    enabled: Boolean(slug) && !(pagesQuery.isSuccess && !isEnabled),
  });

  const record = deploymentQuery.data ?? null;
  if (pagesQuery.isSuccess && !isEnabled) return <NotFound />;
  if (deploymentQuery.isSuccess && !record) return <NotFound />;
    ...
    <div className="min-h-screen bg-background">
      ...
      <main className="mx-auto w-full max-w-6xl space-y-10 px-4 py-10">
        <div className="space-y-3 text-center px-2">
          ...
          <h1 className="text-3xl sm:text-4xl font-bold text-foreground tracking-tight">{record?.title ?? 'Deployment'}</h1>
          {data.intro && (
                ...
                return (
                  <div key={idx} className="grid gap-5 lg:grid-cols-[0.6fr_1.4fr]">
                    <div className="rounded-[32px] border border-border bg-card/70 p-6 shadow-sm flex flex-col justify-center">
                      ...
                      <div className="mt-6">{hasVideo ? <VideoFrame src={inst.videoUrl} label={`Installation Video ${idx + 1}`} /> : <WirePlaceholder label="Installation Video" />}</div>
                    </div>
                    <div className="rounded-[32px] border border-border bg-card/70 p-6 sm:p-8 shadow-sm h-full flex flex-col">
